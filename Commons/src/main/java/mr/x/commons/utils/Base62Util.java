...
public class Base62Util {
	...
	public static String encode(long number) {
		if (number < 0)
			throw new IllegalArgumentException(
					"Number(Base62) must be positive: " + number);
		if (number == 0)
			return "0";
	...
	private static int getIndex(String s, int pos) {
		char c = s.charAt(pos);
		if (c > FAST_SIZE) {
			throw new IllegalArgumentException("Unknow character for Base62: "
					+ s);
		}
	...
	public static void main(String[] args) {
		System.out.println(Base62Util.encode(11297643821283L));
//		System.out.println(Base62Util.decode("DHUfcu4"));
//		System.out.println(Base62Util.decode("DHUfcu4"));
	
//		System.out.println(Base64.encodeBase64URLSafeString("DHUfcu4".getBytes()));
